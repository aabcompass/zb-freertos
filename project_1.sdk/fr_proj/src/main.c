...
#define mainCREATE_TCP_UDP_CLI_TASKS			1
#define mainCLI_TASKS_TCP_MODE					1
...
#define mainRAM_DISK_SECTORS			( ( 20UL * 1024UL * 1024UL ) / mainRAM_DISK_SECTOR_SIZE ) /* 20M bytes. */
...
#define mainHOST_NAME					"k-euso"
#define mainDEVICE_NICK_NAME			"ZynqBoardK"
...
static void prvPeriodicMessageSender( void *pvParameters );
...
static TaskHandle_t xPerMsgSndrTaskHandle = NULL;
...
TimerHandle_t xLEDTimer;
	...
	xil_printf("prvMiscInitialisation done...\r\n");
	...
	***NOTE*** Tasks that use the network are created in the network event hook
	...
	xil_printf("xTaskCreate SvrWork\n\r");
	xTaskCreate(prvServerWorkTask, "SvrWork", mainTCP_SERVER_STACK_SIZE, NULL, tskIDLE_PRIORITY, &xServerWorkTaskHandle );
	xTaskCreate(prvPeriodicMessageSender, "PerMngSndr", mainTCP_SERVER_STACK_SIZE, NULL, tskIDLE_PRIORITY, &xPerMsgSndrTaskHandle );
...
extern void createTestFile( const char *pcMountPath );
	...
	createTestFile( mainRAM_DISK_NAME );
...
static void prvPeriodicMessageSender( void *pvParameters )
{
	...
	for(i=0;i<50;i++)
	{
		FreeRTOS_printf(("Periodic cnt: %i\r\n", i));
		vTaskDelay( 1000 );
	}
...
static BaseType_t xTasksAlreadyCreated = pdFALSE;
	...
	if( eNetworkEvent == eNetworkUp )
	{
		...
		if( xTasksAlreadyCreated == pdFALSE )
		{
			...
			#if (mainCREATE_TCP_UDP_CLI_TASKS==1)
			{
				#if(mainCLI_TASKS_TCP_MODE==1)
				{
					vRegisterSampleCLICommands();
					vRegisterFileSystemCLICommands();
					vRegisterTCPCLICommands();
					vStartTCPCommandInterpreterTask( mainUDP_CLI_TASK_STACK_SIZE, mainUDP_CLI_PORT_NUMBER, mainUDP_CLI_TASK_PRIORITY );
				}
				#else
				{
					vRegisterSampleCLICommands();
